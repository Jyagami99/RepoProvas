import { Category, Test, Term } from "@prisma/client";
...
async function insert(createTestData: CreateTestData) {
  ...
  if (!existingCategory) throw badRequestError("Esta categoria não existe!");

  const existingDiscipline = await disciplineService.getById(disciplineId);
  if (!existingDiscipline) throw badRequestError("Esta disciplina não existe!");

  const existingTeacher = await teacherService.getById(teacherId);
  if (!existingTeacher) throw badRequestError("Este professor não existe!");
  ...
  if (!teacherDiscipline) {
    throw badRequestError("Este professor não ensina essa disciplina!");
  }
...
async function view(id: number) {
  ...
  if (!test) throw notFoundError("Esta prova não existe!");
